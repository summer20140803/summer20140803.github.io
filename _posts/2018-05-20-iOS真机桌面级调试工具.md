...
date:       2018-05-20
author:     开不了口的猫
header-img: img/icon_TDFScreenDebugger_bg.jpg
catalog: true
tags:
    - iOS
    - 真机调试
...
`iOS真机桌面级调试工具`已放入[GitHub Repo](https://github.com/summer20140803/TDFScreenDebugger)中，可以直接clone或通过Cocoapods集成，欢迎Star🤓
...
* 负责维护一套`DispatchQueuePool`，因为附属组件很多都有自己的`常驻监控需求`，因此可以充分利用`多核`优势
* 部分展示数据(APM为主)的更新采用`异步绘制渲染`的机制
* `flowlayout动画`设计以及一些视图细节优化
